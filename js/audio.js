const AudioPlayer = (() => {
  ...
  function load(url) {
    ...
    audio.addEventListener('canplay', () => { els.status.textContent = 'Ready'; });
  ...
  function toggle() {
    ...
    if (audio.paused) {
      els.status.textContent = 'Loading…';
      audio.play().then(() => { showPause(); els.status.textContent = ''; }).catch(() => { els.status.textContent = 'Tap to play'; showPlay(); });
    } else { audio.pause(); showPlay(); }
  }
  ...
  function tryResume(url) {
    try {
      ...
      if (saved?.src === url && saved.time > 0 && audio) audio.currentTime = saved.time;
    } catch {}
  }
